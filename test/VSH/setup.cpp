...
//        d.Vc(VX3,k,j,i) = data.Tlm_phi(1,1,k,j);
        ...
        if (FABS(1.*d.Vc(VX3,k,j,i) - sqrt(2.*3./(8.*M_PI))*COS(x2)*COS(x3)) > 1e-6) {
          countphi ++;
//std::cout << d.Vc(VX3,k,j,i)/sqrt(2.*3./(8.*M_PI))*COS(x2)*COS(x3) << std::endl;
//std::cout << d.Tlm_phi(1,1,k,j).imag()/sqrt(2.*3./(8.*M_PI))*COS(x2)*COS(x3) << std::endl;
////std::cout << d.Vc(VX3,k,j,i) << std::endl;
//std::cout << d.Tlm_phi(1,1,k,j) << std::endl;
//std::cout << std::endl;
std::cout << d.Tlm_phi(1,1,k,j)/(sqrt(2.*3./(8.*M_PI))*COS(x2)*COS(x3)) << std::endl;
        }
        if (FABS(1.*d.Vc(VX2,k,j,i) - sqrt(2.*3./(8.*M_PI))*SIN(x3)) > 1e-6) {
          countth ++;
//std::cout << d.Tlm_th(1,1,k,j)/(sqrt(2.*3./(8.*M_PI))*SIN(x3)) << std::endl;
        }
        if (FABS(1.*d.Vc(VX1,k,j,i) - 0.5*sqrt(15./(2.*M_PI))*pow(SIN(x2),2)*COS(2.*x3)) > 1e-6) {
          countr ++;
std::cout << 2.*d.Vc(VX1,k,j,i)/(0.5*sqrt(15./(2.*M_PI))*pow(SIN(x2),2)*COS(2.*x3)) << std::endl;
std::cout << d.Ylm_r(2,2,k,j) << std::endl;
        }
      }
    }
  } 
